...
namespace BotService
{
	...
	{
        private readonly List<(Type, Func<IBotMessage, IMessageCommand, object>)> _customProperty;
        private readonly List<Func<IChatId, ISendMessages, object>> _commands;
		...
		public ManagerBots(List<IBot> bots, List<(Type, Func<IBotMessage, IMessageCommand, object>)> customProperty, List<Func<IChatId, ISendMessages, object>> commands)
		{
		...
		private void Bot_OnMessage(IBotMessage msg, IBotId botId)
		{
			if (!_chats.TryGetValue(msg.Chat.Id, out var mapper))
			{
                ...
                var chatMapper = new ChatMapper(_bots[botId].TypeBot, msg.Chat.Id, msgColl, _customProperty, _commands);
