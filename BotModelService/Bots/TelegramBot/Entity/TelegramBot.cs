...
using MessageType = BotModel.Bots.BotTypes.Enums.MessageType;

namespace BotModel.Bots.TelegramBot.Entity
{
	public class TelegramBot : IConcurrentBot
	{
        private readonly ITexterService _service;
		...
		public TelegramBot(SecureString token, IBotId id, ITexterService service)
		{
            _service = service;
            Id = id;
		...
		private Texter GetNormalizeText(Texter text, IChatId chatId) => _service.GetNormalizeText(text, chatId);
		...
		public async Task<IBotMessage> SendMessage(IMessageToBot message, IChatId chatId)
		{
			try
			{
